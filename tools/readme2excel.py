...
def paragraph2dicts(paragraph):
    ...
    return [
        {k: v for d in [item2dict(item), school_dict] for k, v in d.items()}
        for item in items
    ]
...
datas = list(reduce(lambda x, y: x + y, map(paragraph2dicts, paragraphs), []))


df = pd.DataFrame(datas)
df.head()
df.to_excel("CSSummerCamp2023.xlsx", index=False)
